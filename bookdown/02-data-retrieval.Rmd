...
```{r simpler-functions, eval=FALSE}
get_head("tblArgoMerge_REP") %>% kable()
get_var_coverage("tblAltimetry_REP", "sla")
get_var_resolution("tblAltimetry_REP", "sla")
get_var_unit("tblAltimetry_REP", "sla")
## get_columns('tblAMT13_Chisholm') %>% kable()
```
...
dat = get_spacetime(tableName = 'tblsst_AVHRR_OI_NRT',
                     varName = 'sst',
                     dt1 = '2016-04-30',
                     dt2 = '2016-04-30',
                     lon1 = 69,
...
```{r manual-query, eval=FALSE}
exec_manualquery("SELECT [time], lat, lon, depth, Fe FROM tblPisces_NRTx
         WHERE
         [time] BETWEEN '2017-06-03' AND '2017-06-03' AND
         lat BETWEEN 10 AND 55 AND
         lon BETWEEN -180 AND 100 AND
         depth BETWEEN 0 AND 0.5
         ORDER BY [time], lat, lon, depth") %>% kable()
```
